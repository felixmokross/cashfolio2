...
export async function getPeriodDateRangeFromPeriod(
  period: Period,
  accountBookId: string,
) {
  const firstBookingDate = await getFirstBookingDate(accountBookId);
...
export async function getPeriodDateRange(
  ...
  return getPeriodDateRangeFromPeriod(await getPeriod(request), accountBookId);
}

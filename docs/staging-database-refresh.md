...
The `cashfolio-app2` runtime image is intentionally slim, but still includes a
minimal Prisma migration payload so Fly's `release_command` can run migrations
from the same image being deployed.
